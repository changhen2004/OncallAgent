...
def _format_markdown(report: EvalReport) -> str:
    lines = [
        ...
        f"- Positive: {report.positive_total}",
        f"- Top1 hits: {report.top1_hits}/{report.positive_total} ({report.top1_hit_rate:.2%})",
        f"- Top3 hits: {report.top3_hits}/{report.positive_total} ({report.top3_hit_rate:.2%})",
        f"- MRR: {report.mrr:.4f}",
        f"- NDCG@{report.ndcg_k}: {report.ndcg:.4f}",
        f"- Negative: {report.negative_total}（误召回 {report.negative_false_positives} 条，"
        f"误召回率 {report.negative_false_positive_rate:.2%}）",
        "",
