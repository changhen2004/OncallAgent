...
- 新增指标：MRR、NDCG@k（默认 k=3）、负例误召回数与误召回率；Top1/Top3 命中率改为基于正例计算。
- 负例支持：`eval/rag_questions.json` 中 `expected_file` 为 null 表示负例（应无召回），统计误召回。
- 评估集扩充：30 → 40 条，新增告警原文（`alert-*`）、日志片段（`log-*`）、口语化提问（`colloquial-*`）与负例（`neg-*`）。
...
- `colloquial-003`：“消息队列堆积了好多消息，一直消费不完怎么办？”命中第 2 名——热榜手册与积压手册共享“消息队列 / Worker 消费”词面，词法检索难以区分，Top3 仍命中。
- `neg-003`：“如何配置 Grafana 的告警通知渠道？”误召回 error-rate / p95 手册——“Grafana / 告警”等词与手册正文重叠，说明近义负例需要向量侧或停用词策略进一步压制。

这些非满分样本正是 MRR / NDCG / 误召回率指标的意义：后续检索优化可直接用该报告对比基线。
